...
## 密钥占位符（{{user@服务}} / {{pass@服务}}）

账号密码不写进命令、也不存进 cm 的任何文件——命令里只写**密钥引用**，执行时
cm 从系统密码管理器读取后替换（macOS 钥匙串；Linux 走 libsecret/secret-tool；
Windows 暂不支持）：

```bash
cm add -a mydb -d "连数据库" -- 'mysql -u {{user@mydb}} -p{{pass@mydb}} -h db.local mydb'
cm add -a api  -d "调接口"  -- 'curl -u {{user@api}}:{{pass@api}} https://api.example.com/me'
```

- `{{user@mydb}}` 读取钥匙串项 `mydb` 的**账号**，`{{pass@mydb}}` 读取其**密码**——
  两者指向同一条钥匙串项（服务名为 `mydb`）；
- **项不存在时现场创建**：执行时提示输入账号（仅当命令用到 `user@`）与密码
  （隐藏输入，不回显），存入系统密码管理器后继续执行，下次直接读取；
- 也可在"钥匙串访问"中手动建：种类选"密码"，名称填服务名（如 `mydb`），
  账户填用户名——读取其他 App 存的项时，macOS 会弹一次授权框，点"始终允许"即可；
- **密码不上屏、不落盘**：参数表单的预览、执行历史（`cm history`）中密钥值一律
  显示为 `••••••`；真实值只出现在最终交给 shell 执行的命令里；
- 只含密钥引用的命令**无需终端输入**，脚本/管道中也可直接执行
  （`cm run mydb < /dev/null` 即可）；此时若钥匙串缺项会明确报错；
- 注意：替换是纯文本替换，密码会出现在被执命令的进程参数中（如 `ps` 可见），
...
密钥引用（`{{user@服务}}` / `{{pass@服务}}`）在表单中显示为 🔑 只读行（标注读取
来源），无输入框；预览中的密钥值以 `••••••` 脱敏，真实值在确认后从系统密码
管理器读取。
